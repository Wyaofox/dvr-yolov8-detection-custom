...
# Fix Windows Python timezone detection issue:
# Force datetime.now() to return correct local time (Asia/Shanghai)
...
# Monkey-patch via module-level replacement
import builtins
...
# Supervision for enhanced detection visualization
...
# Import NiceGUI unified dashboard
import nicegui_dashboard

# Import DingTalk notifier
import dingtalk_notify

# Import zone manager (Supervision-based zone/line detection)
from zone_manager import ZoneManager

# Import video recorder (auto-record clips on zone intrusion)
from video_recorder import VideoRecorder

# Import daily report module
import daily_report
...
def setup_logging():
    ...
    # Create formatter (use now_cst() for correct local time)
    class CSTFormatter(logging.Formatter):
        def formatTime(self, record, datefmt=None):
            ...
            return s
    formatter = CSTFormatter('%(asctime)s - %(levelname)s - %(name)s - %(message)s')
...
def log_detection_details(detections, frame_count, timestamp):
    if ENABLE_DETECTION_LOGGING_TO_FILE:
        for detection in detections:
            ...
            class_idx, confidence = int(row[4]), float(row[5])
...
def frame_processing_thread(
    ...
    def _save_person_photo(track_id, crop):
        try:
            SAVE_DIR = get_current_save_dir()
            path = os.path.join(SAVE_DIR, f"person_{track_id}.jpg")
            cv2.imwrite(path, crop, [cv2.IMWRITE_JPEG_QUALITY, 85])
            main_logger.info(f"Saved person photo: {path}")
        except Exception as e:
            main_logger.error(f"Failed to save person photo: {e}")
    ...
    zone_mgr = None  # Will init after first frame
    recorder = None  # Video recorder (init after first frame)
    ...
    daily_report.init(callback=lambda text: dingtalk_notify.send_text(text))
    ...
    try:
        while not stop_event.is_set() or not frame_queue.empty():
            # Drop stale frames to reduce latency - only process the latest
            frame = None
            ...
            # Initialize zone manager on first frame (only once)
            if zone_mgr is None:
                try:
                    zones_enabled = config.getboolean('zones', 'enabled', fallback=True)
                    if zones_enabled:
                        zone_mgr = ZoneManager(denoised_frame.shape, config=config)
                        main_logger.info("区域检测和计数线已启用")
                        # Initialize video recorder alongside zones
                        recorder = VideoRecorder(
                            save_dir=SAVE_DIR_BASE,
                            fps=25,
                            duration=config.getint('zones', 'clip_duration', fallback=10),
                        )
                except Exception as e:
                    main_logger.warning(f"区域检测初始化失败: {e}")

            # 3) Run YOLO tracking with BoT-SORT (built-in ReID for cross-frame identity)
            results = model.track(
            ...
            if detections.size > 0:
                # Track which IDs have already been reported (dedup)
                reported_ids = getattr(frame_processing_thread, '_reported_ids', {})
                if not hasattr(frame_processing_thread, '_reported_ids'):
                    frame_processing_thread._reported_ids = {}
                    reported_ids = {}
                ...
                # Collect kept detections for Supervision annotation
                kept_boxes_xyxy = []
                kept_confidences = []
                kept_class_ids = []
                kept_track_ids = []
                ...
                for det_idx, det in enumerate(detections):
                    # BoT-SORT returns: [x1, y1, x2, y2, class_idx, confidence, track_id_in_data]
                    # But track_id from boxes.id is more reliable
                    row = det[:7]
                    x1, y1, x2, y2 = row[0], row[1], row[2], row[3]
                    confidence = float(row[5])  # confidence is at index 5
                    class_idx = int(row[4])     # class is at index 4
                    ...
                    # Collect for Supervision annotation (always draw box)
                    kept_boxes_xyxy.append([int(x1), int(y1), int(x2), int(y2)])
                    kept_confidences.append(float(confidence))
                    kept_class_ids.append(int(class_idx))
                    kept_track_ids.append(track_id)

                    # === Simple dedup: same track_id = same person ===
                    is_new_person = True
                    if track_id is not None:
                        if track_id in reported_ids:
                            is_new_person = False
                        else:
                            reported_ids[track_id] = time.time()

                    # Clean expired IDs (120 seconds)
                    now_clean = time.time()
                    expired = [k for k, v in reported_ids.items() if now_clean - v > 120]
                    for k in expired:
                        del reported_ids[k]
                    ...
                    detection_count += 1
                    main_logger.info(f"New detection! Track: #{track_id}, conf: {confidence:.2f}")

                    # Save person photo (one per new track ID)
                    person_crop = _crop_person(denoised_frame, (int(x1), int(y1), int(x2), int(y2)))
                    if person_crop is not None:
                        _save_person_photo(track_id, person_crop)
                    ...
                    detection_info = {
                        ...
                        'track_id': track_id         # ByteTrack ID for dedup
                    }
                    ...
                    # Push to dashboard
                    nicegui_dashboard.add_detection(detection_info)

                    # 5E) Enqueue Telegram alert:
                    # pass the FULL detection_info now
                    telegram_alerts.queue_alert(detection_info)

                    # 5F) Log rectangle info (Supervision handles drawing later)
                    if draw_rectangles:
                        ix1 = max(0, int(x1))
                        iy1 = max(0, int(y1))
                ...
                # 6) Draw detections using Supervision (replaces raw cv2 drawing)
                if any_kept_detection and draw_rectangles:
                    sv_detections = sv.Detections(
                        xyxy=np.array(kept_boxes_xyxy, dtype=np.float32) if kept_boxes_xyxy else np.empty((0, 4)),
                        confidence=np.array(kept_confidences),
                        class_id=np.array(kept_class_ids),
                    )
                    # Add tracker_id for TraceAnnotator (only if all track_ids are valid)
                    try:
                        tids = [int(tid) if tid is not None else -1 for tid in kept_track_ids]
                        if tids and all(t >= 0 for t in tids):
                            sv_detections.tracker_id = np.array(tids, dtype=np.int64)
                    except Exception:
                        pass
                    # Box annotator with rounded corners
                    box_annotator = sv.BoxAnnotator(thickness=2)
                    # Label annotator
                    label_annotator = sv.LabelAnnotator(text_position=sv.Position.TOP_LEFT)
                    labels = []
                    for i, c in enumerate(kept_confidences):
                        # Show persistent ReID identity
                        tid = kept_track_ids[i]
                        id_str = f" #{tid}" if tid is not None else ""
                        c_pct = min(c, 1.0)
                        labels.append(f"Person{id_str} {c_pct:.0%}")
                    denoised_frame = box_annotator.annotate(
                        scene=denoised_frame, detections=sv_detections
                    )
                    denoised_frame = label_annotator.annotate(
                        scene=denoised_frame, detections=sv_detections, labels=labels
                    )
                    # Draw trajectory traces
                    try:
                        denoised_frame = trace_annotator.annotate(
                            scene=denoised_frame, detections=sv_detections
                        )
                    except Exception:
                        pass
                    # Add FPS and detection count overlay
                    _fps_val = total_frames / max(time.time() - last_log_time, 0.001)
                    _fps_text = f"FPS: {_fps_val:.1f}  Detections: {detection_count}  Live: {len(kept_boxes_xyxy)}"
                    cv2.putText(
                        denoised_frame, _fps_text,
                        (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                        (0, 255, 255), 2, cv2.LINE_AA
                    )
                    # Update dashboard FPS
                    nicegui_dashboard.update_fps(_fps_val)
                    # Timestamp overlay
                    cv2.putText(
                        denoised_frame, timestamp_str,
                        (10, denoised_frame.shape[0] - 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                        (0, 255, 255), 2, cv2.LINE_AA
                    )

                    # Zone/Line detection and annotation
                    if zone_mgr is not None:
                        zone_result = zone_mgr.update(sv_detections, fps=_fps_val if '_fps_val' in dir() else None)
                        denoised_frame = zone_mgr.annotate(denoised_frame, sv_detections)
                        # DingTalk zone alerts + video recording + daily report
                        for zone_name, person_count in zone_result.get('zone_alerts', []):
                            dingtalk_notify.send_zone_alert(zone_name, person_count, person_count)
                            if recorder is not None:
                                recorder.start(denoised_frame, reason=zone_name)
                            daily_report.record_zone_event(zone_name)
                ...
                if any_kept_detection:
                    ...
                    # Daily report
                    daily_report.record_detection()
                    # Start TTS in a separate thread if not running
                    if tts_thread is None or not tts_thread.is_alive():
                        tts_stop_event.clear()
                        tts_thread = threading.Thread(target=announce_detection)
                        tts_thread.start()
                    # DingTalk detection alert
                    dingtalk_notify.send_alert(
                        confidence=max(kept_confidences) if kept_confidences else 0.5,
            ...
            if recorder is not None:
                recorder.write_frame(denoised_frame)

            # 8) Send to web server or local display
            if ENABLE_WEBSERVER:
                set_output_frame(denoised_frame)
            # Push frame to Chinese dashboard (compressed, low latency)
            # Push frame to NiceGUI dashboard
            nicegui_dashboard.set_frame(denoised_frame)
            nicegui_dashboard.update_fps(_fps_val if '_fps_val' in dir() else 0)
            if zone_mgr is not None:
                nicegui_dashboard.update_zone_stats(zone_mgr.get_stats())
            if not headless:
...
def signal_handler(sig, frame):
    ...
    # Force destroy all OpenCV windows immediately
    try:
        cv2.destroyAllWindows()
    except Exception:
        pass
...
if __name__ == "__main__":
    ...
    # Start NiceGUI unified dashboard on port 5001
    try:
        nicegui_thread = threading.Thread(
            target=nicegui_dashboard.start_dashboard,
            args=(WEBSERVER_HOST, WEBSERVER_PORT + 1),
            daemon=True,
        )
        nicegui_thread.start()
        main_logger.info(f"NiceGUI dashboard started at http://127.0.0.1:{WEBSERVER_PORT + 1}")
    except Exception as e:
        main_logger.warning(f"NiceGUI dashboard failed to start: {e}")

    # Send config info to dashboard
    nicegui_dashboard.update_config({
        '模型': DEFAULT_MODEL_VARIANT,
        '置信度阈值': DEFAULT_CONF_THRESHOLD,
        'CUDA设备': 'cuda:0',
        '摄像头索引': WEBCAM_INDEX if USE_WEBCAM else 'RTMP',
        '保存目录': str(SAVE_DIR_BASE) if 'SAVE_DIR_BASE' in dir() else './yolo_detections/',
    })
    ...
    dingtalk_notify.init(
        webhook_url=config.get('dingtalk', 'webhook_url', fallback=''),
        enabled=config.getboolean('dingtalk', 'enabled', fallback=False),
        cooldown=config.getint('dingtalk', 'cooldown', fallback=30),
        keyword=config.get('dingtalk', 'keyword', fallback='监控'),
    )
